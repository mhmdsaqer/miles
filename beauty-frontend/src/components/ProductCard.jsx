...
const ProductCard = ({ product }) => {
  ...
  // ✅ دالة الإضافة للسلة - مع منع التداخل مع الرابط
  ...
  return (
    <div className="group relative">
      ...
      >
        ...
        <div className="absolute top-6 inset-x-6 z-10 flex justify-between items-start flex-row-reverse pointer-events-none">
        ...
        {/* ===== Image Container - مع Padding متجاوب ✅ ===== */}
        <div className="aspect-[1/1.1] rounded-[2rem] bg-[#F8F8F8] mb-6 overflow-hidden relative transition-colors duration-700 group-hover:bg-[#F3F3F3] p-4 sm:p-6 md:p-8 lg:p-10">
          <img
          ...
          {/* ===== Add to Cart Button - يظهر على الهوفر (ديسكتوب) ودائمًا (موبايل) ===== */}
          <div className="absolute inset-x-4 bottom-4 translate-y-6 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-out md:group-hover:opacity-100 md:group-hover:translate-y-0 opacity-100 translate-y-0 md:opacity-0 md:translate-y-4">
            <button
        ...
        <div className="px-3 pb-2 text-right space-y-4">
          <h3 className="text-gray-900 font-bold text-[15px] leading-snug line-clamp-2 min-h-[42px] group-hover:text-pink-600 transition-colors duration-300">
            {productName}
          </h3>
          ...
          <div className="flex items-end justify-between pt-2 border-t border-gray-50">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-pink-500 uppercase tracking-[0.2em] mb-1">
                {t('price')}
              </span>
              <span className="text-2xl font-black text-gray-900 tracking-tighter tabular-nums">
                <span className="text-lg ml-0.5">₪</span>{product.price}
              </span>
            </div>
            ...
            {product.has_variants && variantsCount > 0 ? (
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center overflow-hidden">
                  <img 
                    src={getImageUrl(product.options[0].image)} 
                    alt="" 
              ...
              <span className="text-[9px] font-bold text-gray-300 bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
