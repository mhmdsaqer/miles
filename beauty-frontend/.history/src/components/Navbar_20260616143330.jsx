// src/components/Navbar.jsx - نسخة العرض الكامل (Full Width) والارتفاع النحيف العصري 🚀
...
const Navbar = () => {
  ...
  return (
    <>
      <nav 
        // ✅ العرض ممدود بالكامل من الحافة للحافة (fixed top-0 left-0 right-0 w-full) بدون أي padding خارجي (px-0 py-0)
        className="fixed top-0 left-0 right-0 w-full z-[100] transition-all duration-500 pointer-events-none"
        dir={lang === "ar" ? "rtl" : "ltr"}
        role="navigation"
        aria-label="Main navigation"
      >
        {/* ✨ تم إزالة الـ max-width والـ rounded الدائرية ليمتد كامل العرض، وتم تقليص الـ padding العمودي إلى (py-1.5) لارتفاع شديد النحافة */}
        <div className={`w-full transition-all duration-500 pointer-events-auto border-b ${
          ...
          <div className="max-w-[1440px] mx-auto px-4 md:px-8 flex justify-between items-center h-full">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              {/* تقليل الـ padding وحجم الخط هنا ليتناسب مع الارتفاع الصغير */}
            ...
            {/* ===== الجانب الأيسر: الشعار بحجم ملموم ومدروس يمنع تمدد الشريحة عمودياً ===== */}
            <div className="flex-shrink-0">
              <Link to="/" className="group relative flex items-center justify-center" aria-label={t('home')}>
                {/* 📐 تصغير الحاوية الحاضنة للشعار لتلائم النحافة الجديدة للبار */}
                <div className="relative z-10 animate-float-luxury w-20 h-10 md:w-24 md:h-12 lg:w-28 lg:h-14 flex items-center justify-center overflow-hidden bg-transparent">
                  <div className="absolute inset-0 bg-gradient-to-r from-pink-500/10 via-purple-500/10 to-pink-500/10 rounded-full blur-xl opacity-0 group-hover:opacity-60 transition-opacity duration-500 pointer-events-none"></div>
                  <img
                    src={LOGO_URL}
                    alt="Company Logo"
                    className={`
