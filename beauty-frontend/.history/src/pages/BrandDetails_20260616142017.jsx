...
const BrandDetails = () => {
  ...
  return (
    <div className={`min-h-screen bg-[#FAFAFA] pb-24 ${lang === "ar" ? "font-arabic" : "font-latin"}`} dir={lang === "ar" ? "rtl" : "ltr"} lang={lang}>
      ...
      {/* ===== Hero Banner - الهيكل الفخم المتناسق مع الشاشات والـ NavBar 🚀 ===== */}
      {/* تم زيادة الـ pt هنا لعمل إزاحة آمنة ومريحة تنزل التصميم تحت الـ NavBar بالكامل فلا يتداخلان */}
      <div className="relative bg-[#0d0d0d] overflow-hidden pt-20 md:pt-28 lg:pt-32">
        
        {/* حاوية سينمائية تمنع القص العمودي الجائر وتحافظ على استقرار الأبعاد الشاشات الكبيرة والموبايل */}
        <div className="relative w-full aspect-[16/9] sm:aspect-[16/7] md:aspect-[1920/680] lg:aspect-[1920/620] max-h-[580px] flex items-center justify-center overflow-hidden">
          
          {brand.header_image ? (
            <img
              src={getImageUrl(brand.header_image)}
              alt={`${brandDisplayName} Header`}
              // احتواء ذكي عالي الجودة لضمان ظهور كامل تفاصيل الوجه والنصوص السفلية برؤية كاملة
          ...
          {/* تظليل علوي متدرج ومدروس يدمج حافة الـ NavBar الشفاف بخلفية سوداء ناعمة تريح العين */}
          <div className="absolute top-0 left-0 right-0 h-28 bg-gradient-to-b from-black/50 via-black/20 to-transparent pointer-events-none z-20" />

          {/* تظليل سفلي خفيف لعزل نصوص التحكم لتبدو واضحة فوق الصورة الأصلية المضيئة */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent z-15 pointer-events-none" />

          {/* محتوى الواجهة التفاعلي الفاخر */}
          <div className={`
            absolute bottom-0 left-0 right-0 z-30
            max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 
            pb-6 sm:pb-8 md:pb-10 pt-20
          ...
          `}>
            ...
            <div className={`space-y-3 w-full ${lang === "ar" ? "text-right" : "text-left"}`}>
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                {brand.image && (
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-white/95 backdrop-blur-md border border-gray-200/40 flex items-center justify-center p-1.5 shadow-2xl shrink-0">
                    <img
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-white tracking-tighter leading-none italic drop-shadow-[0_3px_12px_rgba(0,0,0,0.7)]">
                    {brandDisplayName}
                  </h1>
            ...
            <button
              ...
              className={`group flex items-center gap-2 bg-black/40 backdrop-blur-md border border-white/10 text-white px-4 py-2.5 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest hover:bg-gray-950 hover:border-gray-950 transition-all shrink-0 shadow-xl ${lang === "ar" ? "flex-row" : "flex-row-reverse"}`}
            >
