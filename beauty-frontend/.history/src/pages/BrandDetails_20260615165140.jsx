...
const BrandDetails = () => {
  ...
  return (
    <div className={`min-h-screen bg-[#FAFAFA] pb-24 ${lang === "ar" ? "font-arabic" : "font-latin"}`} dir={lang === "ar" ? "rtl" : "ltr"} lang={lang}>
      ...
      {/* ===== Hero Banner - النسخة النهائية: صورة كاملة خلف النص + حل مشكلة الـ Navbar ✨ ===== */}
      {/* ✅ ✅ ✅ الحل الجذري: pt-24 md:pt-28 لبدء المحتوى من تحت الـ Navbar + absolute inset-0 على جميع الشاشات */}
      <div className="relative bg-gray-900 overflow-hidden pt-24 md:pt-28 h-[65vh] min-h-[500px] md:h-[75vh] md:min-h-[550px] md:max-h-[900px]">
        
        {/* ✅ 1️⃣ صورة الهيدر - تملأ الحاوية كاملة على جميع الشاشات (بما فيها الموبايل) */}
        {brand.header_image ? (
          <img
            src={getImageUrl(brand.header_image)}
            alt={`${brandDisplayName} Header`}
            // ✅ على جميع الشاشات: absolute inset-0 object-cover object-center
            // ✅ object-[center_30%] لإظهار الجزء العلوي-الأوسط من الصورة (حيث العنصر الرئيسي عادة)
            className="absolute inset-0 w-full h-full object-cover object-[center_30%] transition-transform duration-[2000ms] ease-out hover:scale-105"
            loading="eager"
          />
        ) : (
