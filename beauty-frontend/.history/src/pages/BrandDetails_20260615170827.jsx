...
const BrandDetails = () => {
  ...
  return (
    <div className={`min-h-screen bg-[#FAFAFA] pb-24 ${lang === "ar" ? "font-arabic" : "font-latin"}`} dir={lang === "ar" ? "rtl" : "ltr"} lang={lang}>
      ...
      {/* ===== Hero Banner - التصميم الفاخر الجديد ✨ ===== */}
      {/* ✅ ✅ ✅ الحل الجذري: 
          - pt-24 md:pt-28 لتعويض ارتفاع الـ Navbar (≈96px)
          - aspect-ratio متجاوب لكل شاشة
          - object-cover مع object-[center_25%] لإظهار الجزء العلوي-الأوسط من الصورة
      */}
      <div className="relative bg-gray-900 overflow-hidden pt-24 md:pt-28">
        ...
        <div className="relative w-full aspect-[4/5] sm:aspect-[16/10] md:aspect-[2/1] lg:aspect-auto lg:h-[75vh] lg:min-h-[600px] lg:max-h-[900px]">
          ...
          {/* ✅ 1️⃣ صورة الهيدر - تملأ الحاوية بالكامل */}
          {brand.header_image ? (
            <img
              src={getImageUrl(brand.header_image)}
              alt={`${brandDisplayName} Header`}
              // ✅ object-[center_25%] يركز على الجزء العلوي-الأوسط (حيث العنصر الرئيسي)
              className="absolute inset-0 w-full h-full object-cover object-[center_25%] transition-transform duration-[2000ms] ease-out hover:scale-105"
              loading="eager"
              decoding="async"
            />
          ) : (
          ...
          `}>
            ...
            <div className={`space-y-5 w-full ${lang === "ar" ? "text-right" : "text-left"}`}>
              ...
              <div className="flex items-center gap-3 sm:gap-4 md:gap-6">
                ...
                {brand.image && (
                  <div className="w-14 h-14 sm:w-16 sm:h-16 md:w-24 md:h-24 rounded-xl sm:rounded-2xl md:rounded-3xl bg-white/10 backdrop-blur-2xl border border-white/30 flex items-center justify-center p-2 sm:p-3 md:p-4 shadow-2xl ring-1 ring-white/10">
                    <img
                ...
                {/* اسم البراند */}
                <div>
                  <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-8xl font-black text-white tracking-tighter leading-none italic drop-shadow-[0_5px_20px_rgba(0,0,0,0.8)]">
                    {brandDisplayName}
                  </h1>
            ...
            <button
              ...
              className={`group flex items-center gap-2 sm:gap-3 bg-white/10 backdrop-blur-2xl border border-white/30 text-white px-4 sm:px-5 md:px-6 py-2.5 sm:py-3 md:py-3.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-[11px] font-black uppercase tracking-widest hover:bg-white hover:text-gray-900 transition-all shrink-0 shadow-2xl ring-1 ring-white/10 ${lang === "ar" ? "flex-row" : "flex-row-reverse"}`}
            >
