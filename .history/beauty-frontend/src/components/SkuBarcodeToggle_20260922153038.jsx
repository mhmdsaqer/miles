// src/components/SkuBarcodeToggle.jsx - النسخة النهائية المتجاوبة 💻
...
const SkuBarcodeToggle = ({ sku, barcode }) => {
  ...
  // ✅ الدالة الموحدة للتبديل (تعمل مع hover و click)
  const handleActivate = (item) => {
    if (isMobile) {
      // على الموبايل: toggle بين SKU و Barcode
      setActiveItem(prev => prev === item ? null : item);
    } else {
      // على الديسكتوب: hover عادي
      setActiveItem(item);
    }
  };
  ...
  return (
    <div className="flex items-center justify-center py-3" dir={isRTL ? "rtl" : "ltr"}>
      <div className="relative flex items-center gap-2">
        ...
        {/* ===== بطاقة SKU ===== */}
        <div
          onMouseEnter={() => handleActivate("sku")}
          onMouseLeave={handleDeactivate}
          onClick={() => handleActivate("sku")}
          className={`
        ...
        >
          ...
          `}>
            ...
            {/* سهم التوسيع - يظهر على الموبايل فقط */}
            {isMobile && (
              <div className={`
                transition-all duration-700
        ...
        {/* ===== بطاقة الباركود ===== */}
        <div
          onMouseEnter={() => handleActivate("barcode")}
          onMouseLeave={handleDeactivate}
          onClick={() => handleActivate("barcode")}
          className={`
            relative overflow-hidden cursor-pointer select-none
            transition-all duration-700 ease-out
        ...
        >
          ...
          `}>
            ...
            {isMobile && (
              <div className={`
                transition-all duration-700
                ${activeItem === "barcode" ? "opacity-0 translate-x-4" : "opacity-100 translate-x-0"}
              `}>
                <svg 
        ...
        {/* ✅ تلميح للمستخدم على الموبايل */}
        {isMobile && (
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap">
            <span className="text-[9px] font-bold text-gray-400 bg-white/80 backdrop-blur-sm px-2 py-1 rounded-full border border-gray-200 shadow-sm">
              {lang === "ar" ? "اضغط للتبديل" : "Tap to switch"}
            </span>
          </div>
        )}
      </div>
