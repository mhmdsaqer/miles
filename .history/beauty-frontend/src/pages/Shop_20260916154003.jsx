// src/pages/Shop.jsx - النسخة النهائية (فاتح فقط - Light Mode Only)
...
const Shop = () => {
  ...
  const FiltersContent = () => (
    <>
      {/* Categories Tree */}
      <div className="rounded-[2rem] p-6 border shadow-sm bg-white border-gray-50">
        <h3 className="text-[10px] font-black uppercase tracking-[0.3em] mb-5 text-gray-900">
          {t("categories")}
        </h3>
  ...
  if (error) {
    return (
      <div className="min-h-screen pt-32 px-6 flex items-center justify-center bg-[#fcfcfc]" dir={lang === "ar" ? "rtl" : "ltr"}>
        <div className="text-center space-y-6 max-w-md">
          ...
          <button 
            ...
            className="px-6 py-3 rounded-xl font-bold text-sm transition-all bg-gray-900 text-white hover:bg-pink-600"
          >
            {t('tryAgain')}
          </button>
        </div>
  ...
  if (loading && products.length === 0) {
    return (
      <div className="min-h-screen pt-32 px-6 lg:px-12 bg-[#fcfcfc]" dir={lang === "ar" ? "rtl" : "ltr"}>
        <div className="max-w-[1600px] mx-auto">
          <div className="h-16 w-64 rounded-2xl animate-pulse mb-8 bg-gray-100" />
          <div className="flex flex-col lg:flex-row gap-16">
            <div className="w-full lg:w-64 space-y-4">
              {[...Array(6)].map((_, i) => <div key={i} className="h-10 rounded-xl animate-pulse bg-gray-100" />)}
            </div>
            <div className="flex-1">
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                {[...Array(8)].map((_, i) => <div key={i} className="aspect-[4/5] rounded-[2.5rem] animate-pulse bg-gray-100" />)}
              </div>
            </div>
          </div>
        </div>
  ...
  // ===== Main Content =====
  return (
    <div className="min-h-screen pt-32 bg-[#fcfcfc]" dir={lang === "ar" ? "rtl" : "ltr"}>
      <div className="max-w-[1600px] mx-auto px-6 lg:px-12">
        ...
        <div className="mb-8 pb-6 border-b border-gray-100">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              ...
              <nav className="flex items-center gap-2 text-[10px] font-bold text-pink-500 uppercase tracking-[0.2em]">
                ...
                {activeFilters.find(f => ["parent", "child", "grandchild"].includes(f.type)) && (
                  <>
                    ...
                    <span className="text-gray-900">{activeFilters.find(f => ["parent", "child", "grandchild"].includes(f.type))?.label}</span>
                  </>
                )}
              </nav>
            ...
            <button
              ...
              className="md:hidden flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold border transition-colors bg-white border-gray-200 text-gray-900 hover:border-pink-300 hover:text-pink-600"
              aria-label={t("filter")}
            >
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4">
            <div className="relative flex-1 min-w-[200px] max-w-md">
              <input
                ...
                className="w-full border rounded-2xl pl-12 pr-5 py-3.5 text-sm outline-none focus:ring-2 focus:ring-pink-500/20 transition-colors bg-white border-gray-100 text-gray-900 placeholder-gray-300"
              />
              <svg className={`absolute ${lang === "ar" ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 w-5 h-5 text-gray-300`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="relative min-w-[180px]">
              <select 
                ...
                className="appearance-none border rounded-2xl px-4 py-3.5 text-[11px] font-bold w-full outline-none focus:ring-2 focus:ring-pink-500/20 transition-colors bg-white border-gray-100 text-gray-900"
              >
          ...
          {activeFilters.length > 0 && (
            <div className="mt-4 flex flex-wrap items-center gap-2">
              {activeFilters.map((f, i) => (
                <span key={i} className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-bold bg-pink-50 text-pink-700">
                  {f.label}
                  <button onClick={() => clearFilter(f.type)} className="rounded-full w-4 h-4 flex items-center justify-center transition hover:bg-pink-200">✕</button>
                </span>
              ))}
              <button onClick={clearAllFilters} className="text-xs underline text-gray-400 hover:text-gray-600">{t("clearAll")}</button>
            </div>
          )}
        </div>
        ...
        {filters.parent && (
          <div className="mb-8 p-5 rounded-[2rem] border shadow-sm bg-white border-gray-100">
            ...
            <div className="flex overflow-x-auto pb-2 scrollbar-hide gap-2">
              <button
                ...
                className="px-5 py-2.5 rounded-xl text-xs font-bold transition-all border whitespace-nowrap bg-gray-900 text-white border-gray-900 shadow-md"
              >
                {lang === "ar" ? "الكل" : "All"}
              </button>
            ...
            {products.length > 0 ? (
              ...
              !loading && (
                <div className="h-[50vh] flex flex-col items-center justify-center text-center">
                  <h2 className="text-2xl font-black mb-2 text-gray-900">{t("noResults")}</h2>
            ...
            {hasNextPage && (
              <div className="mt-16 text-center">
                <button
                  ...
                  className="inline-flex items-center gap-3 border px-8 py-4 rounded-2xl text-sm font-bold transition-all disabled:opacity-50 bg-white border-gray-200 text-gray-700 hover:border-pink-500 hover:text-pink-600"
                >
      ...
      {showFilters && !isDesktop && (
        <>
          ...
          <div 
            className="fixed z-[999] top-0 right-0 h-full w-[85%] max-w-sm shadow-2xl transition-transform duration-300 ease-out animate-slideIn bg-[#fcfcfc]"
            role="dialog"
            aria-modal="true"
            aria-label={t("filter")}
          >
            {/* Drawer Header */}
            <div className="sticky top-0 z-10 flex items-center justify-between p-5 border-b bg-[#fcfcfc] border-gray-100">
              ...
              <button
                onClick={() => setShowFilters(false)}
                className="w-10 h-10 rounded-xl flex items-center justify-center transition-colors bg-gray-100 hover:bg-gray-200 text-gray-900"
                aria-label={t("close")}
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="sticky bottom-0 p-4 border-t bg-[#fcfcfc] border-gray-100">
              <div className="flex gap-3">
                {activeFilters.length > 0 && (
                  <button
                    ...
                    className="flex-1 py-3.5 rounded-xl text-xs font-bold transition-colors bg-gray-100 text-gray-700 hover:bg-gray-200"
                  >
                    {t("clearAll")}
                  </button>
                )}
