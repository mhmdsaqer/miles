...
const AdminProducts = () => {
  ...
  const openModal = useCallback(async (product = null) => {
    ...
    if (product) {
      ...
      setFormData({
        ...product,
        barcode: product.barcode || "",  // ✅ جديد
        brand_id: String(product.brand_id || ""),
        category_id: String(product.category_id || ""),
        sku: product.sku ? String(product.sku).toUpperCase() : "",
        description_ar: product.description_ar || "",
        description_en: product.description_en || "",
        ingredients_ar: product.ingredients_ar || "",  // ✅ جديد
        ingredients_en: product.ingredients_en || "",  // ✅ جديد
        usage_ar: product.usage_ar || "",              // ✅ جديد
        usage_en: product.usage_en || "",              // ✅ جديد
      ...
      if (product.sku) {
          toast.info(
            <div className="flex items-center gap-3">
              <span className="text-xl">💡</span>
              <div>
                <p className="font-bold text-sm">{lang === "ar" ? "نصيحة" : "Tip"}</p>
                <p className="text-xs text-gray-500">
                  {lang === "ar" 
                    ? "يمكنك إضافة متغيرات لهذا المنتج. المتغير الأول سيحمل نفس SKU المنتج الأصلي."
                    : "You can add variants to this product. The first variant will have the same SKU as the original product."}
                </p>
              </div>
            </div>,
            { duration: 4000 }
  ...
  const handleConfirmPromote = useCallback(async () => {
    ...
    try {
      ...
      const payload = {
        ...
        description_ar: promoteData.description_ar?.trim() || null,
        description_en: promoteData.description_en?.trim() || null,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const variantsPayload = variants.length > 0 
        ? variants.map((v, index) => {
            ...
            return {
              ...
              sku: finalSku,
              // ✅ ✅ ✅ تحويل فراغ المتغير إلى null
              barcode: v.barcode?.trim() ? v.barcode.trim().toUpperCase() : null,              price: Number(v.price) || Number(formData.price),
      ...
      const productPayload = {
        ...
        barcode: formData.barcode?.trim() ? formData.barcode.trim().toUpperCase() : null,
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input name="id" type="number" value={formData.id} onChange={handleChange} placeholder="ID" required disabled={!!editingId} className={`border rounded-xl px-4 py-2.5 text-sm transition-colors ${
                    isDark 
