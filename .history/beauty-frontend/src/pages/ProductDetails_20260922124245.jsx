// src/pages/ProductDetails.jsx - النسخة المُصححة (فاتح فقط - Light Mode Only) ✅
...
const ProductDetails = () => {
  ...
  // ✅ ✅ ✅ جديد: ضبط التاب النشط تلقائياً بناءً على أول عمود متاح يحتوي على بيانات
  useEffect(() => {
        if (product) {
          ...
          // ترتيب الأولوية: الوصف > المكونات > طريقة الاستخدام
          if (hasDesc) {
            ...
            setActiveTab(null); // لا يوجد أي محتوى
          }
        }
      }, [product, lang]);
  useEffect(() => {
  const fetchAndFilter = async () => {
    try {
      ...
      // ✅ ✅ ✅ الحل الجذري: دمج المنتج الأصلي كأول "متغير افتراضي" في الواجهة
      let finalOptions = current.options || [];
      
      if (current.has_variants) {
        const parentAsVariant = {
          id: `parent_default_${current.id}`, // معرف خاص لتمييزه برمجياً
          isParentDefault: true,               // علامة مميزة نستخدمها لاحقاً
          sku: current.sku,
          barcode: current.barcode,
          price: current.price,
          image: current.image,
          attributes: {
            [lang === "ar" ? "النوع" : "Type"]: lang === "ar" ? "المنتج الأصلي" : "Original Product"
          },
          isAvailable: current.isAvailable !== false
        };
        // نضعه في بداية المصفوفة ليكون هو الخيار الأول دائماً
        finalOptions = [parentAsVariant, ...finalOptions];
      }

      // نحدّث حالة المنتج بالخيارات المدمجة
      setProduct({ ...current, options: finalOptions });

      // تحديد المتغير الأول المتاح كخيار افتراضي
      if (finalOptions.length > 0) {
        const firstAvailableVariant = finalOptions.find(opt => opt.isAvailable !== false) || finalOptions[0];
      ...
      // ... (باقي كود جلب المنتجات المشابهة كما هو) ...
        ...
        <div className="w-full lg:w-[45%] p-6 lg:p-16 flex flex-col justify-center bg-white">
          <div className="max-w-lg mx-auto w-full space-y-8">
            <header className="space-y-5 text-right">
              ...
              {/* ✅ ✅ ✅ تم إصلاح هذا القسم: إزالة isDark وتثبيت تصميم Light Mode الأنيق */}
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl border bg-gray-50 border-gray-100">
