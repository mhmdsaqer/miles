...
// ==========================================
// ⚠️ ⚠️ ⚠️ مؤقت: لتحديث المنتجات القديمة (احذف هذا الكود بعد استخدامه مرة واحدة!) ⚠️ ⚠️ ⚠️
// ==========================================
app.get("/api/temp-update-products", async (req, res) => {
  try {
    const Product = require("./models/Product");
    
    // ✅ القيمة الثابتة التي ستظهر في حال لم يقم الأدمن بإدخال شيء
    // (ملاحظة: إذا كنت تفضل أن تكون null تماماً، غيّر النصوص أدناه إلى null)
    const defaultIngredientsAr = "المكونات مطبوعة على عبوة المنتج";
    const defaultIngredientsEn = "Ingredients are printed on the product packaging";
    const defaultUsageAr = "طريقة الاستخدام مطبوعة على عبوة المنتج";
    const defaultUsageEn = "Usage instructions are printed on the product packaging";

    const result = await Product.updateMany(
      {}, // {} تعني: حدّث جميع المنتجات الموجودة في قاعدة البيانات
      {
        $set: {
          ingredients_ar: defaultIngredientsAr,
          ingredients_en: defaultIngredientsEn,
          usage_ar: defaultUsageAr,
          usage_en: defaultUsageEn
        }
      }
    );
