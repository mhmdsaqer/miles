...
app.get("/search-by-barcode", async (req, res) => {
  ...
  // 1. ابحث في المنتجات
  const productsWithBarcode = await Product.find({ barcode: normalizedBarcode });
  
  // 2. ابحث في المتغيرات
  const variantsWithBarcode = await Variant.find({ barcode: normalizedBarcode });
  
  // 3. ارجع كل النتائج
  res.json({
    barcode: normalizedBarcode,
    products: productsWithBarcode,
    variants: variantsWithBarcode,
    totalMatches: productsWithBarcode.length + variantsWithBarcode.length
  });
});
