...
// 🔧 🔧 🔧 إصلاح مؤقت لمرة واحدة: حذف الـ Index القديم المسبب لتكرار null 🔧 🔧 🔧
Product.collection.dropIndex("barcode_1").catch(() => console.log("✅ تم حذف فهرس barcode القديم (Products) أو لم يكن موجوداً"));
Variant.collection.dropIndex("barcode_1").catch(() => console.log("✅ تم حذف فهرس barcode القديم (Variants) أو لم يكن موجوداً"));

// ✅ السماح لـ Mongoose بإعادة بناء الـ Indexes بشكل صحيح (مع sparse: true)
Product.syncIndexes().catch(err => console.log("⚠️ Product indexes sync note:", err.message));
Variant.syncIndexes().catch(err => console.log("⚠️ Variant indexes sync note:", err.message));
