  ...
  async (req, res) => {
    try {
      const {
        ...
        description_ar, description_en, image, price, sku, barcode,
        ingredients_ar, ingredients_en, usage_ar, usage_en, // ✅ أضف هذا السطر
      ...
      const newProduct = new Product({
        ...
        description_en: description_en?.trim() || null,
          // ✅ ✅ ✅ جديد: حفظ القيم مع تحويل الفراغ إلى null
      ...
      await audit.create(req.user, "product", newProduct.toObject(), req);
            // ✅ حفظ المتغيرات إذا وجدت
      if (variants && Array.isArray(variants) && variants.length > 0) {
        const parentSku = finalSku;
        // ✅ التحقق مما إذا كان الفرونت إند أرسل المنتج الأصلي كمتغير ضمن المصفوفة
        const hasParentInPayload = variants.some(v => v.sku?.toUpperCase().trim() === parentSku);
        
        const variantsToSave = [];
        
        // ✅ 1. إذا لم يرسل الفرونت إند المنتج الأصلي، نقوم بإضافته نحن تلقائياً كأول متغير
        if (!hasParentInPayload) {
          variantsToSave.push({
            id: Date.now(),
            product_id: id,
            sku: parentSku,
            ...
            price: Number(price),
            image: image,
            attributes: {}, // يمكن للواجهة عرضه كـ "المنتج الأصلي"
            isAvailable: req.body.isAvailable !== undefined ? req.body.isAvailable : true
          });
        }

        // ✅ 2. إضافة المتغيرات الجديدة التي أرسلها المستخدم
        variants.forEach((v, index) => {
          const variantSku = v.sku?.trim()?.toUpperCase() || `${parentSku}-${String(index + 1).padStart(3, '0')}`;
          const variantBarcode = v.barcode?.trim()?.toUpperCase() || null;  
          variantsToSave.push({
            id: v.id && !String(v.id).startsWith('temp_') ? Number(v.id) : (Date.now() + index + 1000),
            product_id: id,
            sku: variantSku,
        ...
        // ✅ 3. التحقق من عدم تكرار الـ SKU بين المتغيرات
  ...
  async (req, res) => {
    try {
      ...
      const { variants, sku, ...productData } = req.body;
            ...
            // ✅ ✅ ✅ جديد: تنظيف الحقول الجديدة قبل الحفظ في قاعدة البيانات
      productData.ingredients_ar = productData.ingredients_ar?.trim() || null;
  ...
  async (req, res) => {
    try {
      ...
      if (id) {
       ...
       // ✅ ✅ ✅ 3. التحقق من تكرار الـ SKU - مع السماح بتطابق SKU المنتج الأصلي
      if (sku) {
        const normalizedSku = sku.toUpperCase().trim();
        const parentSku = parentProduct.sku?.toUpperCase().trim();
        
        // ✅ إذا كان SKU المتغير يطابق SKU المنتج الأصلي، نسمح بذلك (للمتغير الأساسي)
        if (normalizedSku === parentSku) {
          ...
          if (existingVariant) {
            return res.status(400).json({
              message: `⚠️ SKU "${normalizedSku}" موجود مسبقاً كمتغير آخر لهذا المنتج`
            });
          }
        } else {
          // ✅ إذا كان SKU مختلفاً، نتحقق من عدم تكراره بين المتغيرات
      ...
      // ✅ ✅ ✅ جديد: التحقق من الـ Barcode
            let finalBarcode = req.body.barcode?.trim()?.toUpperCase() || null;
